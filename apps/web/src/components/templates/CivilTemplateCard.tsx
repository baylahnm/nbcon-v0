...
import { Star, Users, Eye, Building2 } from "lucide-react";
import { CivilTemplate } from "@/pages/templates/civil-templates/types/civil-templates";
import { cn } from "@/lib/utils";

interface CivilTemplateCardProps {
  template: CivilTemplate;
  onPreview?: (template: CivilTemplate) => void;
}

const categoryColors: Record<string, string> = {
  "site-design": "bg-blue-500/10 text-blue-600 border-blue-500/20",
  "material-estimation": "bg-green-500/10 text-green-600 border-green-500/20",
...
export function CivilTemplateCard({ template, onPreview }: CivilTemplateCardProps) {
  const categoryColor = categoryColors[template.civilCategory] || categoryColors["site-design"];
  const categoryLabel = categoryLabels[template.civilCategory] || template.civilCategory;
      ...
      <div className="aspect-square w-full overflow-hidden bg-muted/30 relative">
        {template.previewImage ? (
          ...
          <div className="aspect-square h-full w-full flex items-center justify-center bg-gradient-to-br from-muted/50 to-muted">
            <Building2 className={cn("h-12 w-12", template.categoryColor)} />
          </div>
        )}
        {/* Featured Badge */}
        {template.featured && (
