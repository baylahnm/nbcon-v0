...
import { Star, Users, Eye, Zap } from "lucide-react";
import { ElectricalTemplate } from "@/pages/templates/electrical-templates/types/electrical-templates";
import { cn } from "@/lib/utils";

interface ElectricalTemplateCardProps {
  template: ElectricalTemplate;
  onPreview?: (template: ElectricalTemplate) => void;
}

const categoryColors: Record<string, string> = {
  "load-schedules": "bg-yellow-500/10 text-yellow-600 border-yellow-500/20",
  "panel-design": "bg-blue-500/10 text-blue-600 border-blue-500/20",
  "wiring-plans": "bg-green-500/10 text-green-600 border-green-500/20",
};
...
export function ElectricalTemplateCard({ template, onPreview }: ElectricalTemplateCardProps) {
  const categoryColor = categoryColors[template.electricalCategory] || categoryColors["load-schedules"];
  const categoryLabel = categoryLabels[template.electricalCategory] || template.electricalCategory;
      ...
      <div className="aspect-square w-full overflow-hidden bg-muted/30 relative">
        {template.previewImage ? (
          ...
          <div className="aspect-square h-full w-full flex items-center justify-center bg-gradient-to-br from-muted/50 to-muted">
            <Zap className={cn("h-12 w-12", template.categoryColor)} />
          </div>
        )}
        {/* Featured Badge */}
        {template.featured && (
