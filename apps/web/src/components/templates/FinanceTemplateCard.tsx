...
import { Star, Users, Eye, DollarSign, CheckCircle2 } from "lucide-react";
import { FinanceTemplate } from "@/pages/templates/finance-templates/types/finance-templates";
import { cn } from "@/lib/utils";

interface FinanceTemplateCardProps {
  template: FinanceTemplate;
  onPreview?: (template: FinanceTemplate) => void;
}

const categoryColors: Record<string, string> = {
  "cost-estimation": "bg-green-500/10 text-green-600 border-green-500/20",
  "budget-management": "bg-blue-500/10 text-blue-600 border-blue-500/20",
  "financial-analysis": "bg-purple-500/10 text-purple-600 border-purple-500/20",
};
...
export function FinanceTemplateCard({ template, onPreview }: FinanceTemplateCardProps) {
  const categoryColor = categoryColors[template.financeCategory] || categoryColors["cost-estimation"];
  const categoryLabel = categoryLabels[template.financeCategory] || template.financeCategory;
      ...
      <div className="aspect-square w-full overflow-hidden bg-muted/30 relative">
        {template.previewImage ? (
          ...
          <div className="aspect-square h-full w-full flex items-center justify-center bg-gradient-to-br from-muted/50 to-muted">
            <DollarSign className={cn("h-12 w-12", template.categoryColor)} />
          </div>
        )}
        {/* Featured Badge */}
        {template.featured && (
      ...
      <div className="flex-1 flex flex-col p-4 min-w-0 text-card-foreground">
        ...
        {template.contributor && (
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs font-medium">{template.contributor.name}</span>
            {template.contributor.verified && (
