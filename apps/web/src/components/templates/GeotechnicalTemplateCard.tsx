...
import { Star, Users, Eye, Layers } from "lucide-react";
import { GeotechnicalTemplate } from "@/pages/templates/geotechnical-templates/types/geotechnical-templates";
import { cn } from "@/lib/utils";

interface GeotechnicalTemplateCardProps {
  template: GeotechnicalTemplate;
  onPreview?: (template: GeotechnicalTemplate) => void;
}

const categoryColors: Record<string, string> = {
  "soil-analysis": "bg-amber-500/10 text-amber-600 border-amber-500/20",
  "foundation-design": "bg-blue-500/10 text-blue-600 border-blue-500/20",
  "slope-stability": "bg-green-500/10 text-green-600 border-green-500/20",
};
...
export function GeotechnicalTemplateCard({ template, onPreview }: GeotechnicalTemplateCardProps) {
  const categoryColor = categoryColors[template.geotechnicalCategory] || categoryColors["soil-analysis"];
  const categoryLabel = categoryLabels[template.geotechnicalCategory] || template.geotechnicalCategory;
      ...
      <div className="aspect-square w-full overflow-hidden bg-muted/30 relative">
        {template.previewImage ? (
          ...
          <div className="aspect-square h-full w-full flex items-center justify-center bg-gradient-to-br from-muted/50 to-muted">
            <Layers className={cn("h-12 w-12", template.categoryColor)} />
          </div>
        )}
        {/* Featured Badge */}
        {template.featured && (
