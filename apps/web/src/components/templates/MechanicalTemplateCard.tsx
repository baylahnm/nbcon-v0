...
import { Star, Users, Eye, Cog } from "lucide-react";
import { MechanicalTemplate } from "@/pages/templates/mechanical-templates/types/mechanical-templates";
import { cn } from "@/lib/utils";

interface MechanicalTemplateCardProps {
  template: MechanicalTemplate;
  onPreview?: (template: MechanicalTemplate) => void;
}

const categoryColors: Record<string, string> = {
  hvac: "bg-purple-500/10 text-purple-600 border-purple-500/20",
  piping: "bg-blue-500/10 text-blue-600 border-blue-500/20",
  "mechanical-systems": "bg-green-500/10 text-green-600 border-green-500/20",
};
...
export function MechanicalTemplateCard({ template, onPreview }: MechanicalTemplateCardProps) {
  const categoryColor = categoryColors[template.mechanicalCategory] || categoryColors.hvac;
  const categoryLabel = categoryLabels[template.mechanicalCategory] || template.mechanicalCategory;
      ...
      <div className="aspect-square w-full overflow-hidden bg-muted/30 relative">
        {template.previewImage ? (
          ...
          <div className="aspect-square h-full w-full flex items-center justify-center bg-gradient-to-br from-muted/50 to-muted">
            <Cog className={cn("h-12 w-12", template.categoryColor)} />
          </div>
        )}
        {/* Featured Badge */}
        {template.featured && (
