...
import { Star, Users, Eye, Navigation } from "lucide-react";
...
const categoryColors: Record<string, string> = {
  gnss: "bg-blue-500/10 text-blue-600 border-blue-500/20",
  lidar: "bg-green-500/10 text-green-600 border-green-500/20",
  photogrammetry: "bg-purple-500/10 text-purple-600 border-purple-500/20",
  "field-forms": "bg-orange-500/10 text-orange-600 border-orange-500/20",
};
...
export function SurveyTemplateCard({ template, onPreview }: SurveyTemplateCardProps) {
  const categoryColor = categoryColors[template.surveyCategory] || categoryColors.gnss;
  const categoryLabel = categoryLabels[template.surveyCategory] || template.surveyCategory;
      ...
      <div className="aspect-square w-full overflow-hidden bg-muted/30 relative">
        {template.previewImage ? (
          ...
          <div className="aspect-square h-full w-full flex items-center justify-center bg-gradient-to-br from-muted/50 to-muted">
            <Navigation className={cn("h-12 w-12", template.categoryColor)} />
          </div>
        )}
        {/* Featured Badge */}
        {template.featured && (
