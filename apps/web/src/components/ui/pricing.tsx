	...
	return (
		...
		>
			<div className="mx-auto max-w-xl space-y-2">
				<h2 className="text-center subsection-heading">
					{heading}
				</h2>
				{description && (
			...
			/>
			<div className="container mx-auto grid w-full grid-cols-1 gap-4 px-4 sm:px-6 lg:px-8 md:grid-cols-2 lg:grid-cols-4">
				{plans.map((plan) => (
		...
		>
			...
			>
				<div className="absolute top-2 right-2 z-10 flex items-center gap-2">
					{plan.highlighted && (
						<p className="bg-background flex items-center gap-1 rounded-md border border-border dark:border-border-elevated px-2 py-0.5 text-xs">
							<StarIcon className="h-3 w-3 fill-current" />
							Popular
						</p>
					)}
					{frequency === 'yearly' && plan.price.monthly > 0 && (
						<p className="bg-primary text-primary-foreground flex items-center gap-1 rounded-md border border-border dark:border-border-elevated px-2 py-0.5 text-xs">
							{Math.round(
				...
				<h3 className="mt-2 flex items-end gap-1">
					{plan.price[frequency] === 0 ? (
						...
						<>
							<span className="text-3xl font-bold">SAR {plan.price[frequency]}</span>
			...
			<div
				className={cn(
					'mt-auto w-full border-t border-border dark:border-border-elevated bg-background p-3',
					plan.highlighted && 'bg-muted/40',
				)}
			>
				<Button
